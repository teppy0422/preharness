...
- efu_detail.dart を読み込んだ時にアプリと端子が異なる場合の処理を追加
- efu_detail.dart を読み込んだ時にマイクロメーターが null の場合の処理を追加
...
- ドラムロールピッカーを追加
  ...
  OK。次に入力インターフェイスを作りたい。3 桁の数字。1 から 255 までの数字。
  デフォルト値は 001。入力は上下のフリック。フリックはそれぞれ 3 つの数字毎に
  持つ。下に OK と CANCEL ボタン。モーダル表示。タイトルは"CFM 番号を選択"。
