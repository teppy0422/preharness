...
class GlowBorderRow extends StatefulWidget {
  ...
  const GlowBorderRow({
    ...
    this.glowColor = Colors.lightGreenAccent,
  });

  @override
  State<GlowBorderRow> createState() => _GlowBorderRowState();
}

class _GlowBorderRowState extends State<GlowBorderRow>
...
class _GlowBorderPainter extends CustomPainter {
  final double progress; // 0.0〜1.0 で1周の進捗
  final double borderWidth;
  final Color borderColor;
  final Color glowColor;
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // 外枠を描く
    canvas.drawRect(rect.deflate(borderWidth / 2), basePaint);

    // 4辺の長さ合計
    final double width = rect.width - borderWidth;
    final double height = rect.height - borderWidth;
    final double perimeter = 2 * (width + height);

    // 光る点の位置（perimeter上の距離）
    final double distance = perimeter * progress;

    // 光る点のサイズ
    const double glowRadius = 8;
    ...
    if (distance <= width) {
      // 上辺：左→右
      point = Offset(borderWidth / 2 + distance, borderWidth / 2);
    } else if (distance <= width + height) {
      // 右辺：上→下
      double d = distance - width;
      point = Offset(rect.width - borderWidth / 2, borderWidth / 2 + d);
    } else if (distance <= 2 * width + height) {
      // 下辺：右→左
      double d = distance - width - height;
      point = Offset(
        rect.width - borderWidth / 2 - d,
        rect.height - borderWidth / 2,
      );
    } else {
      // 左辺：下→上
      double d = distance - 2 * width - height;
      point = Offset(borderWidth / 2, rect.height - borderWidth / 2 - d);
    }

    // グロー（光る点）を描く（半透明の円でふわっと）
    final Paint glowPaint = Paint()
      ..color = glowColor.withOpacity(0.7)
      ..maskFilter = MaskFilter.blur(BlurStyle.normal, 10);

    canvas.drawCircle(point, glowRadius.toDouble(), glowPaint);

    // 中心色の明るい小さい円
    final Paint centerPaint = Paint()..color = glowColor.withOpacity(1.0);

    canvas.drawCircle(point, glowRadius / 2, centerPaint);
  }
